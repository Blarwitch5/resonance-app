...
export function controlInsetClass(options: { leading?: boolean | "search"; trailing?: boolean } = {}): string {
  const start =
    options.leading === "search" ? "pl-9 sm:pl-12" : options.leading ? "pl-10 sm:pl-11" : "pl-3 sm:pl-4";
...
export const controlIconSlotClass =
  "flex w-9 shrink-0 items-center justify-center text-text-tertiary sm:w-12";

...
export function RecordSides({ sides, artist, title, coverUrl, locale = "en", keepClose }: RecordSidesProps) {
  ...
  const listen = pressingListen(sides, locale);
...
function StaticRecordSides({ sides, locale = "en" }: RecordSidesProps) {
    ...
    <div className="flex flex-col gap-6">
      {sides.map((side, sideIndex) => (
        <div key={`${side.heading ?? "side"}-${sideIndex}`} className="flex flex-col gap-3">
          <RecordSideHeading side={side} showRuntime={showRuntime} locale={locale} />
          <ol className="flex flex-col" aria-label={side.heading ?? t(locale, "sample.tracks")}>
